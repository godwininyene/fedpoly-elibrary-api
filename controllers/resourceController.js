...
/**
 * GET /api/resources
 * No changes needed here — listing doesn't affect counts.
 */
 ...
 * Unique view count — only increments once per user per resource.
 */
export const getResourceById = async (req, res, next) => {
  try {
    ...
    // Internally checks uniqueness before incrementing.
    trackInteraction({
      userId: req.user._id,
 ...
 * Unique download count — only increments once per user per resource.
 */
export const getDownloadUrl = async (req, res, next) => {
  try {
    const resource = await Resource.findById(req.params.id)
      .select('publicId title isPublished');
    ...
    const { url, expiresAt } = generateSignedDownloadUrl(resource.publicId, 300);

    // Fire-and-forget unique download tracking
    trackInteraction({
      userId: req.user._id,
      resourceId: resource._id,
      action: 'download',
...
export const deleteResource = async (req, res, next) => {
  try {
    ...
    // Also clean up all interaction records for this resource
    // so counts don't linger for non-existent resources
